...
FALLBACK_THEME_COLORS_FLET = {
    "primary": ft.Colors.GREEN_ACCENT_700,
    "secondary": ft.Colors.BLUE_700,
    "surface": "#282828", 
    "on_surface": ft.Colors.WHITE,
    "error": ft.Colors.RED_700,
    "success": ft.Colors.GREEN_700,
    "text_secondary": ft.Colors.with_opacity(0.75, ft.Colors.WHITE),
    "border_color": ft.Colors.with_opacity(0.2, ft.Colors.WHITE),
    "amber_accent_700": ft.Colors.AMBER_ACCENT_700,
    "transparent": ft.Colors.TRANSPARENT,
    "black": ft.Colors.BLACK, 
    "chart_color_1": ft.Colors.GREEN_ACCENT_700, 
    "chart_color_2": ft.Colors.BLUE_700,       
    "chart_color_3": ft.Colors.LIGHT_GREEN_ACCENT_700,
    "chart_color_4": ft.Colors.LIGHT_BLUE_ACCENT_700,
    "chart_color_5": ft.Colors.LIME_ACCENT_700,
    "surface_opacity_005": ft.Colors.with_opacity(0.05, "#282828"),      
    "on_surface_opacity_008": ft.Colors.with_opacity(0.08, ft.Colors.WHITE),
    "secondary_opacity_03": ft.Colors.with_opacity(0.3, ft.Colors.BLUE_700),
    "primary_opacity_01": ft.Colors.with_opacity(0.1, ft.Colors.GREEN_ACCENT_700), 
    "black_with_opacity_85": ft.Colors.with_opacity(0.85, ft.Colors.BLACK), 
    "on_surface_opacity_015": ft.Colors.with_opacity(0.15, ft.Colors.WHITE) 
}
...
def create_product_card(
    ...
    ct = theme_colors or FALLBACK_THEME_COLORS_FLET
    
    primary_color = ct.get("primary", FALLBACK_THEME_COLORS_FLET["primary"])
    secondary_color = ct.get("secondary", FALLBACK_THEME_COLORS_FLET["secondary"])
    on_surface_color = ct.get("on_surface", FALLBACK_THEME_COLORS_FLET["on_surface"])
    text_secondary_color = ct.get("text_secondary", FALLBACK_THEME_COLORS_FLET["text_secondary"])
    card_bgcolor = ct.get("on_surface_opacity_008", FALLBACK_THEME_COLORS_FLET["on_surface_opacity_008"])
    border_color_card = ct.get("secondary_opacity_03", FALLBACK_THEME_COLORS_FLET["secondary_opacity_03"])
    image_container_bgcolor = ct.get("surface_opacity_005", FALLBACK_THEME_COLORS_FLET["surface_opacity_005"])
    star_icon_color = ct.get("amber_accent_700", FALLBACK_THEME_COLORS_FLET["amber_accent_700"])
    divider_transparent_color = ct.get("transparent", FALLBACK_THEME_COLORS_FLET["transparent"])

    handle_details_click = on_details_click if callable(on_details_click) else lambda _p: logger.debug(f"Card details N/A: {product.get('name')}")
    handle_compare_click = on_compare_click if callable(on_compare_click) else lambda _p: logger.debug(f"Card compare N/A: {product.get('name')}")
    ...
    image_container = ft.Container(
        content=ft.Image(src=product.get("image_url", "https://via.placeholder.com/200x200.png?text=No+Img"),fit=ft.ImageFit.CONTAIN,error_content=ft.Icon(name=ft.Icons.BROKEN_IMAGE_OUTLINED, size=40, color=text_secondary_color)),
...
def create_price_chart(products: list[dict], theme_colors: dict | None = None):
    ...
    chart_palette = [
        ct.get("chart_color_1"), ct.get("chart_color_2"), ct.get("chart_color_3"),
        ct.get("chart_color_4"), ct.get("chart_color_5")
    ]
    chart_palette = [color for color in chart_palette if color is not None]
    if not chart_palette: 
        chart_palette = [FALLBACK_THEME_COLORS_FLET["primary"]] 

    tooltip_bgcolor_val = ct.get("black_with_opacity_85")
    grid_lines_color_val = ct.get("on_surface_opacity_015", ft.Colors.with_opacity(0.15, ft.Colors.WHITE)) 
    ...
    chart_widget = ft.BarChart(bar_groups=bar_groups,bottom_axis=ft.ChartAxis(labels=bottom_axis_labels, labels_size=50),left_axis=ft.ChartAxis(labels=left_axis_labels,
    labels_size=35,title=ft.Text("Цена (₽)", color=on_surface_color, size=11, weight=ft.FontWeight.BOLD),title_size=20),horizontal_grid_lines=ft.ChartGridLines(interval=y_step, color=grid_lines_color_val, width=1),
    tooltip_bgcolor=tooltip_bgcolor_val,interactive=True,height=280,expand=True)
